...
const func: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
    ...
    if (!isEnableMockPriceProxy()) {
        ...
        if (!ethers.utils.isAddress(pythPriceProxyAddress as string)) return;
        ...
        const switchboardPriceProxyAddress = switchboardPriceProxyInst.address;
        if (!ethers.utils.isAddress(switchboardPriceProxyAddress as string)) return;
    ...
    console.log("\r\n\r\n");
    logger.color('blue').log("-------------------------------------------------");
    logger.color('blue').bold().log("Set PriceOracle in LockersProxy...");

    const lockersProxyInst = await getContractInst(deployments, "LockersProxy", "LockersLogic", "LockersLib");
    if (!lockersProxyInst) return;

    const priceOracleAddress = priceOracleInst.address;
    if (!ethers.utils.isAddress(priceOracleAddress as string)) return;

    const oldPriceOracleAddress = await lockersProxyInst.priceOracle();
    if (oldPriceOracleAddress != priceOracleAddress) {
        const tx = await lockersProxyInst.setPriceOracle(priceOracleAddress);
        await tx.wait(1);
        console.log("PriceOracle setted in lockersProxy, tx hash is", tx.hash);
        console.log("PriceOracle address of lockersProxy is", await lockersProxyInst.priceOracle());
    } else {
        console.log("LockersProxy priceOracle is alreay setted");
    }
};
